...
DECLARE @DropDb BIT = 0; -- Forge: create-only; installer runs only when tables are absent -- Set this to 0 to skip DROP statements, 1 to include them

IF @DropDb = 1
BEGIN
    -- drop indexes if they exist and rebuild if current ones
    DROP INDEX IF EXISTS [IDX_QRTZ_J_G_N] ON [dbo].[QRTZ_JOB_DETAILS];
    ...
    DROP INDEX IF EXISTS [IDX_QRTZ_T_G_N] ON [dbo].[QRTZ_TRIGGERS];
    ...
    IF OBJECT_ID(N'[dbo].[QRTZ_PAUSED_JOB_GRPS]', N'U') IS NOT NULL
    DROP TABLE [dbo].[QRTZ_PAUSED_JOB_GRPS];
...
ALTER TABLE [dbo].[QRTZ_PAUSED_JOB_GRPS] WITH NOCHECK ADD
  CONSTRAINT [PK_QRTZ_PAUSED_JOB_GRPS] PRIMARY KEY  CLUSTERED
  (
    [SCHED_NAME],
    [JOB_GROUP]
  );
